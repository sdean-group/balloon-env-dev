...
def evaluate(ckpt: str, heldout: str, layout: str, *, blocks_per_month: int, seeds: int,
             ...
             s_churn: float = 0.0, sigma_max: float | None = None) -> dict:
    import healpy as hp
    import zarr
    sampler = HpxSampler(ckpt, layout, num_steps=num_steps, device=device, s_churn=s_churn, sigma_max=sigma_max)
    ...
    res["num_steps"], res["s_churn"], res["sigma_max"] = int(num_steps), float(s_churn), float(sampler.sigma_max)
...
def main(argv=None) -> None:
    ...
    ap.add_argument("--sigma-max", type=float, default=None, help="override the checkpoint's starting noise level")
